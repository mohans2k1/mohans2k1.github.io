...
export function getCurrentTimeInIndia(): Date {
  ...
  const offsetIndiaHours = 5; // Italy is in Central European Summer Time (UTC+2), but you might need to adjust this based on Daylight Saving Time
  const offsetIndiaMins = 30;
  now.setHours(now.getUTCHours() + offsetIndiaHours);
  now.setMinutes(now.getUTCHours() + offsetIndiaMins);
